...
ActiveRecord::Schema[7.1].define(version: 2024_10_26_035831) do
  ...
  create_view "unified_printings", materialized: true, sql_definition: <<-SQL
      WITH card_cycles_summary AS (
           SELECT c_1.id,
              array_agg(cc_1.id ORDER BY cc_1.date_release DESC) AS card_cycle_ids,
              array_agg(cc_1.name ORDER BY cc_1.date_release DESC) AS card_cycle_names
             FROM (((cards c_1
           ...
           SELECT c_1.id,
              array_agg(cs_1.id ORDER BY cs_1.date_release DESC) AS card_set_ids,
              array_agg(cs_1.name ORDER BY cs_1.date_release DESC) AS card_set_names
             FROM ((cards c_1
               JOIN printings p_1 ON (((c_1.id)::text = p_1.card_id)))
               JOIN card_sets cs_1 ON ((p_1.card_set_id = (cs_1.id)::text)))
            GROUP BY c_1.id
